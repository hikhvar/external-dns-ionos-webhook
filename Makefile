...
ARTIFACT_NAME = external-dns-inwx-webhook
...
run:build ## Run the binary on local machine
	LOG_LEVEL=$(LOG_LEVEL) LOG_ENVIRONMENT=$(LOG_ENVIRONMENT) LOG_FORMAT=$(LOG_FORMAT) build/bin/external-dns-inwx-webhook
...
release-check: ## Check if the release will work
	GITHUB_SERVER_URL=github.com GITHUB_REPOSITORY=hikhvar/external-dns-inwx-webhook REGISTRY=$(REGISTRY) IMAGE_NAME=$(IMAGE_NAME) $(GORELEASER) release --snapshot --clean --skip=publish
